#==================================================================================================#
# This is the master function for robust subset selection. It calls mio and gd to perform
...
#' @param params an optional list of additional Gurobi parameters (the parameters Time and
#' OutputFlag are controlled by \code{time} and \code{output})
...
rss.fit <- function(x, y,
                    ...
                    int = TRUE, k.mio = NULL, h.mio = NULL, time = 60, tau = 1.25, output = FALSE,
                    params = NULL, robust = TRUE, max.iter.ns = 1e2, max.iter.gd = 1e5, tol = 1e-4,
                    ...) {
  ...
  y.o <- y
  if (robust) {
    x <- rob.scale(x, center = int, scale = TRUE)
    if (any(is.na(x))) stop('At least one predictor has MAD = 0 causing standardisation to fail.')
    y <- rob.scale(y, center = int, scale = FALSE)
  } else {
    x <- scale(x, center = int, scale = TRUE)
    y <- scale(y, center = int, scale = FALSE)
  }
  ...
  for (j in 1:n.h) {
    for (i in 1:n.k) {
      result <- gd(x, y, rep(0, p), rep(0, n), k[i], h[j], step, max.iter.gd, tol)
      ...
      for (i in 1:n.k) {
        for (a in (i - 1):(i + 1)) {
          ...
          for (b in (j - 1):(j + 1)) {
            ...
            result <- gd(x, y, beta.init, eta.init, k[i], h[j], step, max.iter.gd, tol)
  ...
  for (j in 1:n.h) {
    for (i in 1:n.k) {
      if (k[i] != 0) {
        ...
        if(int) {
          beta <- c(0, beta)
          beta[c(TRUE, id.beta)] <- stats::lsfit(x.o[!id.eta,
                                                     id.beta], y.o[!id.eta], int = TRUE)$coef
        } else {
          beta[id.beta] <- stats::lsfit(x.o[!id.eta, id.beta], y.o[!id.eta], int = FALSE)$coef
        }
      } else {
        if (int) {
          eta <- H(y.o, n - h[j])
...
mio <- function(x, y, k, h, init, time, tau, output, params) {
  ...
  if (form == 2) {
    ...
    model$Q <- 0.5 * bdiag(Matrix(0, p + n + p + n, p + n + p + n), Diagonal(n, 1)) # Matrix Q
    # in objective function
    model$obj <- c(- t(W) %*% y, rep(0, p + n + n)) # Vector c in objective function
    model$objcon <- 0.5 * t(y) %*% y # Constant a in objective function
    model$A <- # LHS of constraints
      rbind(
        cbind(Matrix(0, 1, p), Matrix(0, 1, n), Matrix(1, 1, p), Matrix(0, 1, n),
              Matrix(0, 1, n)), # C1
        cbind(Matrix(0, 1, p), Matrix(0, 1, n), Matrix(0, 1, p), Matrix(1, 1, n),
              Matrix(0, 1, n)), # C2
        cbind(Diagonal(p, 1), Matrix(0, p, n), Diagonal(p, - Mb), Matrix(0, p, n),
              Matrix(0, p, n)), # C3
        cbind(Diagonal(p, - 1), Matrix(0, p, n), Diagonal(p, - Mb), Matrix(0, p, n),
              Matrix(0, p, n)), # C4
        cbind(Matrix(0, n, p), Diagonal(n, 1), Matrix(0, n, p), Diagonal(n, - Me),
              Matrix(0, n, n)), # C5
        cbind(Matrix(0, n, p), Diagonal(n, - 1), Matrix(0, n, p), Diagonal(n, - Me),
              Matrix(0, n, n)), # C6
...
plot.rss.fit <- function(x, ...) {
  ...
  beta <- x$beta
  if (x$int) beta <- beta[- 1, , , drop = FALSE]
  beta[beta == 0] <- NA
